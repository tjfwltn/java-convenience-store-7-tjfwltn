...
- 프로모션 혜택은 프로모션 재고 내에서만 적용할 수 있다. (프로모션 재고가 떨어지면 프로모션 적용 불가)
- 프로모션 기간 중이라면 프로모션 재고를 우선적으로 차감하며, 프로모션 재고가 부족할 경우에는 일반 재고를 사용한다.
  (`products.md` 파일에 프로모션이 적용되는 상품들은 프로모션 상품과 일반 상품으로 나뉘어져 있음)
...
### 멤버십 할인 기능 요구 사항
- 멤버십 회원은 프로모션 미적용 금액의 30%를 할인받는다. (N+1를 안한 상품의 총 가격)
...
```console
    예시 : 콜라가 2+1 프로모션인 경우, 3으로 나뉘어지지 않을 때, 
    ex) [콜라-10] -> 10 = (2 + 1) * 3 + 1 이므로 2개를 더 추가할건지 아닌지
    Y
```
- 프로모션 재고가 부족하여 일부 수량을 프로모션 혜택 없이 결제해야 하는 경우,
